# PSBT Howto for Metaworldcoin Core

Since Metaworldcoin Core 0.17, an RPC interface exists for Partially Signed Metaworldcoin
...
PSBT is an interchange format for Metaworldcoin transactions that are not fully signed
...
#### Multisig with multiple Metaworldcoin Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Metaworldcoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `metaworldcoin-cli` is used.
