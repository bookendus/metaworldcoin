...
- [Development guidelines](#development-guidelines)
    - [General Metaworldcoin Core](#general-metaworldcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Metaworldcoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play metaworldcoins" on the test network, if you
...
Metaworldcoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/metaworldcoin-project/metaworldcoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_metaworldcoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_metaworldcoin --log_level=test_suite
$ valgrind -v --leak-check=full src/metaworldcoind -printtoconsole
...
# A coverage report will now be accessible at `./test_metaworldcoin.coverage/index.html`.
```
...
To profile a running metaworldcoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep metaworldcoind` -- sleep 60
```
...
Metaworldcoin Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Metaworldcoin Core, where everyone uses
...
pay attention to for reviewers of Metaworldcoin Core code.

General Metaworldcoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Metaworldcoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of Metaworldcoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against Metaworldcoin Core so that they can be integrated
...
file descriptors, it will cause problems with Metaworldcoin's `select()` loop, because
...
compatibility between nodes. This happened in Metaworldcoin 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Metaworldcoin's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `metaworldcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `metaworldcoin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Metaworldcoin Core, the wallet was always
