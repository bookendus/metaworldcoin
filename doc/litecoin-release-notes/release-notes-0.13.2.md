Metaworldcoin Core version 0.13.2 is now available from:

  <https://download.metaworldcoin.org/metaworldcoin-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a metaworldcoin
...
ECDSA signatures inside Metaworldcoin transactions now use validation using
...
Previous versions of Metaworldcoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Metaworldcoin Core was to raise the default minimum
relay fee.

Metaworldcoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Metaworldcoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running metaworldcoind without having to do any manual configuration.
...
Metaworldcoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Metaworldcoin Core will relay transactions with
...
entirely in Metaworldcoin Core 0.13, and it is at this time undecided whether the
...
Metaworldcoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Metaworldcoin Core automatically creates a hidden service to listen on, without
manual configuration. Metaworldcoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Metaworldcoin Core is listening, and
...
Metaworldcoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Metaworldcoin
Core to use floating fees. By default, Metaworldcoin Core will use floating
fees.
...
(default: `0.0002` MWC/kB).

At all times, Metaworldcoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) MWC.
Furthermore, Metaworldcoin Core will never create transactions paying less than
the current minimum relay fee.
Finally, a user can set the minimum fee rate for all transactions with
`-mintxfee=<i>`, which defaults to 1000 atoms per kB.
...
`metaworldcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /metaworldcoinrpc>
        ...
        # optional bypass metaworldcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< metaworldcoinrpc:<password>
    </Location>
...
Restarting metaworldcoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in metaworldcoin.conf
...
metaworldcoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/metaworldcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Metaworldcoin Core code base has
...
- `metaworldcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `metaworldcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Metaworldcoin scripting system that in combination with
...
Metaworldcoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Metaworldcoin Core.
...
The two phases are distinct in the Metaworldcoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Metaworldcoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Metaworldcoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Metaworldcoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of metaworldcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of metaworldcoins being spent is exactly
...
- **Increased security for multisig:** Metaworldcoin addresses (both P2PKH addresses
  ...
  believed to be provided by Metaworldcoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Metaworldcoin's current ledger (called the UTXO set),
  ...
  note that Metaworldcoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Metaworldcoin users to individually opt-in to almost any change in the Metaworldcoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Metaworldcoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Metaworldcoin
client to Metaworldcoin Core 0.13.2 to signal for segwit support. When a
...
Since Metaworldcoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `metaworldcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `metaworldcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
