Metaworldcoin Core version 0.14.2 is now available from:

  <https://download.metaworldcoin.org/metaworldcoin-0.14.2.0/>
...
Metaworldcoin Core now supports P2SH addresses beginning with M on mainnet and Q on testnet.
...
This only affects users that have explicitly enabled UPnP through the GUI setting or through the -upnp option, as since the last UPnP vulnerability (in Metaworldcoin Core 0.10.4) it has been disabled by default.
...
Testnet3 has been deprecated and replaced with Testnet4. The server port has been changed to 19435 however the RPC port remains
the same (19432).

Testnet faucets can be located at:
- http://testnet.metaworldcointools.com
- http://testnet.thrasher.io

Developers who require the new testnet blockchain paramaters can find them [here](https://github.com/metaworldcoin-project/metaworldcoin/blob/master/src/chainparams.cpp#L220).
...
Metaworldcoin Core has supported automatically pruning the blockchain since 0.13.2. Pruning
...
now ZMQ can be used on Windows. Please see [this document](https://github.com/metaworldcoin-project/metaworldcoin/blob/master/doc/zmq.md) for
...
When Metaworldcoin Core is out-of-sync on startup, a semi-transparent information
...
Commands sent over the JSON-RPC interface and through the `metaworldcoin-cli` binary
can now use named arguments. This follows the [JSON-RPC specification](http://www.jsonrpc.org/specification)
for passing parameters by-name with an object.

`metaworldcoin-cli` has been updated to support this by parsing `name=value` arguments
when the `-named` option is given.

Some examples:

    src/metaworldcoin-cli -named help command="help"
    src/metaworldcoin-cli -named getblockhash height=0
    src/metaworldcoin-cli -named getblock blockhash=000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f
    src/metaworldcoin-cli -named sendtoaddress address="(snip)" amount="1.0" subtractfeefromamount=true
...
The Alert System was disabled and deprecated in Metaworldcoin Core 0.10.4 and removed in 0.13.2. 
...
----------------------
 ...
 - A new RPC command `getmemoryinfo` has been added which will return information
   about the memory usage of Metaworldcoin Core. This was added in conjunction with
...
- calculation of threshold for a dust output. (effectively 3 * 1000 atoms/kB)
- minimum fee rate of a package of transactions to be included in a block created by the mining code. If miners wish to set this minimum they can use the new `-blockmintxfee` option.  (defaults to 1000 atoms/kB)
...
here, originally written for Bitcoin but can also be used for Metaworldcoin:
