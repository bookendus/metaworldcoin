    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional MWC outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes MWC and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::MWC_TO_MWC) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        // First try to construct a MWC-to-MWC transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::MWC_ONLY;
        ...
        // If MWC-to-MWC fails, create a peg-out transaction
