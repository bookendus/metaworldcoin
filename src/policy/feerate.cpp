...
CAmount CFeeRate::GetFee(size_t nBytes_) const
{
    ...
    CAmount nFee = nAtomsPerK * nSize / 1000;
...
bool CFeeRate::MeetsFeePerK(const CAmount& min_fee_per_k) const
{
    ...
    // Anything beyond that can be used to calculate nAtomsPerK.
    CAmount mweb_fee = CAmount(m_weight) * BASE_MWEB_FEE;
    if (m_weight > 0 && m_nFeePaid < mweb_fee) {
        return false;
    }

    // MWEB-to-MWEB transactions don't have a size to calculate nAtomsPerK.
    // Since we got this far, we know the transaction meets the minimum MWEB fee, so return true.
    if (m_nBytes == 0 && m_weight > 0) {
        return true;
    }

    return nAtomsPerK >= min_fee_per_k;
}

std::string CFeeRate::ToString(const FeeEstimateMode& fee_estimate_mode) const
{
    switch (fee_estimate_mode) {
    case FeeEstimateMode::SAT_VB: return strprintf("%d.%03d %s/vB", nAtomsPerK / 1000, nAtomsPerK % 1000, CURRENCY_ATOM);
    default:                      return strprintf("%d.%08d %s/kvB", nAtomsPerK / COIN, nAtomsPerK % COIN, CURRENCY_UNIT);
    }
}
