...
const char * const BITCOIN_CONF_FILENAME = "metaworldcoin.conf";
...
#else
    const char* pszModule = "metaworldcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Metaworldcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Metaworldcoin";
#else
    // Unix
    return pathRet / ".metaworldcoin";
#endif
#endif
}
